...
class TestPatientService(unittest.TestCase):
    ...
    def test_push_to_elastic_search(self):
        test_index = "reporting-test"
        patient_service = PatientService(index=test_index)
        result = patient_service.push_to_elastic_search(self.test_patient_with_encounters.get_json(), index=test_index)
        self.assertTrue(result['created'])
        id = result['_id']
        es = ElasticSearchHandler()
        result = es.get_from_es(test_index, id)
        self.assertTrue(result['found'])
        es.delete_by_id(test_index, 'Patient', id)
